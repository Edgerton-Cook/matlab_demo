%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% TITLE:   autogen/matlab/classes/autogen_led_cmd_solid_t.m               %
% AUTHOR:  Miki Szmuk                                                     %
% LAB:     Autonomous Controls Lab (ACL)                                  %
% LICENSE: Copyright 2016, 2017, All Rights Reserved                      %
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

classdef autogen_led_cmd_solid_t < handle
  properties (Access = public, Constant = true)
    type(1,1) autogen_type_t = autogen_type_t.LED_CMD_SOLID;
  end
  properties (Access = public)
    topic(1,1) autogen_topic_led_cmd_solid_t = autogen_topic_led_cmd_solid_t.UNDEFINED;
  ...
  methods (Sealed = true)
    ...
    function [buffer,i2] = serialize_content(obj,buffer,i1)
      ...
      mustBePositive(i1)
      if (all(isa(buffer,'uint8')))&&(isvector(buffer))
        buffer(i1+0) = typecast(obj.global,'uint8');
        buffer(i1+1) = typecast(obj.red,'uint8');
        buffer(i1+2) = typecast(obj.green,'uint8');
        buffer(i1+3) = typecast(obj.blue,'uint8');
        i2 = i1+4;
      else
        error('Buffer must be a uint8 array.')
      end
    ...
    function [i2] = deserialize_content(obj,buffer,i1)
      ...
      mustBePositive(i1)
      if (all(isa(buffer,'uint8')))&&(isvector(buffer))
        obj.global = typecast(buffer(i1+0),'uint8');
        obj.red = typecast(buffer(i1+1),'uint8');
        obj.green = typecast(buffer(i1+2),'uint8');
        obj.blue = typecast(buffer(i1+3),'uint8');
        i2 = i1+4;
      else
        error('Buffer must be a uint8 array.')
      end
