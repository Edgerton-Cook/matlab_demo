%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% TITLE:   autogen/matlab/classes/autogen_qcontrol_module_data_t.m        %
% AUTHOR:  Miki Szmuk                                                     %
% LAB:     Autonomous Controls Lab (ACL)                                  %
% LICENSE: Copyright 2016, 2017, All Rights Reserved                      %
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

classdef autogen_qcontrol_module_data_t < handle
  properties (Access = public, Constant = true)
    type(1,1) autogen_type_t = autogen_type_t.QCONTROL_MODULE_DATA;
  end
  properties (Access = public)
    topic(1,1) autogen_topic_qcontrol_module_data_t = autogen_topic_qcontrol_module_data_t.UNDEFINED;
    node(1,1) autogen_node_t = autogen_node_t.UNDEFINED;
    ts(1,1) uint64;
    state(1,1) autogen_qcontrol_state_t;
    mode(1,1) autogen_qcontrol_mode_t;
    sig(1,1) autogen_qcontrol_sig_data_t;
    local(1,1) autogen_qcontrol_local_data_t;
    config(1,1) autogen_qcontrol_config_data_t;
  end
  methods (Static)
    function [n] = size()
      n = 11+11305;
  ...
  methods (Sealed = true)
    ...
    function [buffer,i2] = serialize_content(obj,buffer,i1)
      ...
      if (all(isa(buffer,'uint8')))&&(isvector(buffer))
        ...
        buffer = obj.local.serialize_content(buffer,i1+146);
        buffer = obj.config.serialize_content(buffer,i1+210);
        i2 = i1+11305;
      else
        error('Buffer must be a uint8 array.')
      end
    ...
    function [i2] = deserialize_content(obj,buffer,i1)
      ...
      if (all(isa(buffer,'uint8')))&&(isvector(buffer))
        ...
        obj.local.deserialize_content(buffer,i1+146);
        obj.config.deserialize_content(buffer,i1+210);
        i2 = i1+11305;
      else
        error('Buffer must be a uint8 array.')
      end
