%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% TITLE:   autogen/matlab/classes/autogen_max11644eua_module_data_t.m     %
% AUTHOR:  Miki Szmuk                                                     %
% LAB:     Autonomous Controls Lab (ACL)                                  %
% LICENSE: Copyright 2016, 2017, All Rights Reserved                      %
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

classdef autogen_max11644eua_module_data_t < handle
  properties (Access = public, Constant = true)
    type(1,1) autogen_type_t = autogen_type_t.MAX11644EUA_MODULE_DATA;
  end
  properties (Access = public)
    topic(1,1) autogen_topic_max11644eua_module_data_t = autogen_topic_max11644eua_module_data_t.UNDEFINED;
    node(1,1) autogen_node_t = autogen_node_t.UNDEFINED;
    ts(1,1) uint64;
    state(1,1) autogen_max11644eua_state_t;
    meas(1,1) autogen_max11644eua_meas_data_t;
    local(1,1) autogen_max11644eua_local_data_t;
  end
  methods (Static)
    function [n] = size()
      n = 11+13;
  ...
  methods (Sealed = true)
    ...
    function [buffer,i2] = serialize_content(obj,buffer,i1)
      ...
      if (all(isa(buffer,'uint8')))&&(isvector(buffer))
        ...
        buffer = obj.local.serialize_content(buffer,i1+9);
        i2 = i1+13;
      else
        error('Buffer must be a uint8 array.')
      end
    ...
    function [i2] = deserialize_content(obj,buffer,i1)
      ...
      if (all(isa(buffer,'uint8')))&&(isvector(buffer))
        ...
        obj.local.deserialize_content(buffer,i1+9);
        i2 = i1+13;
      else
        error('Buffer must be a uint8 array.')
      end
