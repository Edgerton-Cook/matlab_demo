%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% TITLE:   autogen/matlab/classes/autogen_led_cmd_progress_t.m            %
% AUTHOR:  Miki Szmuk                                                     %
% LAB:     Autonomous Controls Lab (ACL)                                  %
% LICENSE: Copyright 2016, 2017, All Rights Reserved                      %
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

classdef autogen_led_cmd_progress_t < handle
  properties (Access = public, Constant = true)
    type(1,1) autogen_type_t = autogen_type_t.LED_CMD_PROGRESS;
  end
  properties (Access = public)
    topic(1,1) autogen_topic_led_cmd_progress_t = autogen_topic_led_cmd_progress_t.UNDEFINED;
    node(1,1) autogen_node_t = autogen_node_t.UNDEFINED;
    ts(1,1) uint64;
    frac_complete(1,1) single;
    global(1,1) uint8;
    red(1,1) uint8;
    green(1,1) uint8;
    blue(1,1) uint8;
  ...
  methods (Sealed = true)
    ...
    function [buffer,i2] = serialize_content(obj,buffer,i1)
      ...
      mustBePositive(i1)
      if (all(isa(buffer,'uint8')))&&(isvector(buffer))
        buffer(i1+(0:(0+3))) = typecast(obj.frac_complete,'uint8');
        buffer(i1+4) = typecast(obj.global,'uint8');
        buffer(i1+5) = typecast(obj.red,'uint8');
        buffer(i1+6) = typecast(obj.green,'uint8');
        buffer(i1+7) = typecast(obj.blue,'uint8');
        i2 = i1+8;
      else
        error('Buffer must be a uint8 array.')
      end
    ...
    function [i2] = deserialize_content(obj,buffer,i1)
      ...
      mustBePositive(i1)
      if (all(isa(buffer,'uint8')))&&(isvector(buffer))
        obj.frac_complete = typecast(buffer(i1+(0:(0+3))),'single');
        obj.global = typecast(buffer(i1+4),'uint8');
        obj.red = typecast(buffer(i1+5),'uint8');
        obj.green = typecast(buffer(i1+6),'uint8');
        obj.blue = typecast(buffer(i1+7),'uint8');
        i2 = i1+8;
      else
        error('Buffer must be a uint8 array.')
      end
