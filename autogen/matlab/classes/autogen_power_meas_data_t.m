%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% TITLE:   autogen/matlab/classes/autogen_power_meas_data_t.m             %
% AUTHOR:  Miki Szmuk                                                     %
% LAB:     Autonomous Controls Lab (ACL)                                  %
% LICENSE: Copyright 2016, 2017, All Rights Reserved                      %
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

classdef autogen_power_meas_data_t < handle
  properties (Access = public, Constant = true)
    type(1,1) autogen_type_t = autogen_type_t.POWER_MEAS_DATA;
  end
  properties (Access = public)
    topic(1,1) autogen_topic_power_meas_data_t = autogen_topic_power_meas_data_t.UNDEFINED;
    node(1,1) autogen_node_t = autogen_node_t.UNDEFINED;
    ts(1,1) uint64;
    input_voltage(1,1) single;
    input_current(1,1) single;
  end
  methods (Static)
    function [n] = size()
      n = 11+8;
  ...
  methods (Sealed = true)
    ...
    function [buffer,i2] = serialize_content(obj,buffer,i1)
      ...
      mustBePositive(i1)
      if (all(isa(buffer,'uint8')))&&(isvector(buffer))
        buffer(i1+(0:(0+3))) = typecast(obj.input_voltage,'uint8');
        buffer(i1+(4:(4+3))) = typecast(obj.input_current,'uint8');
        i2 = i1+8;
      else
        error('Buffer must be a uint8 array.')
      end
    ...
    function [i2] = deserialize_content(obj,buffer,i1)
      ...
      mustBePositive(i1)
      if (all(isa(buffer,'uint8')))&&(isvector(buffer))
        obj.input_voltage = typecast(buffer(i1+(0:(0+3))),'single');
        obj.input_current = typecast(buffer(i1+(4:(4+3))),'single');
        i2 = i1+8;
      else
        error('Buffer must be a uint8 array.')
      end
