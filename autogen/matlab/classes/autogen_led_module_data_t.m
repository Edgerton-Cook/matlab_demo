%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% TITLE:   autogen/matlab/classes/autogen_led_module_data_t.m             %
% AUTHOR:  Miki Szmuk                                                     %
% LAB:     Autonomous Controls Lab (ACL)                                  %
% LICENSE: Copyright 2016, 2017, All Rights Reserved                      %
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

classdef autogen_led_module_data_t < handle
  properties (Access = public, Constant = true)
    type(1,1) autogen_type_t = autogen_type_t.LED_MODULE_DATA;
  end
  properties (Access = public)
    topic(1,1) autogen_topic_led_module_data_t = autogen_topic_led_module_data_t.UNDEFINED;
    node(1,1) autogen_node_t = autogen_node_t.UNDEFINED;
    ts(1,1) uint64;
    state(1,1) autogen_led_state_t;
    cmd(1,1) autogen_led_cmd_data_t;
    config(1,1) autogen_led_config_data_t;
  end
  methods (Static)
    function [n] = size()
      n = 11+138;
  ...
  methods (Sealed = true)
    ...
    function [buffer,i2] = serialize_content(obj,buffer,i1)
      ...
      if (all(isa(buffer,'uint8')))&&(isvector(buffer))
        ...
        buffer = obj.config.serialize_content(buffer,i1+125);
        i2 = i1+138;
      else
        error('Buffer must be a uint8 array.')
      end
    ...
    function [i2] = deserialize_content(obj,buffer,i1)
      ...
      if (all(isa(buffer,'uint8')))&&(isvector(buffer))
        ...
        obj.config.deserialize_content(buffer,i1+125);
        i2 = i1+138;
      else
        error('Buffer must be a uint8 array.')
      end
