%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% TITLE:   autogen/matlab/classes/autogen_max11644eua_local_data_t.m      %
% AUTHOR:  Miki Szmuk                                                     %
% LAB:     Autonomous Controls Lab (ACL)                                  %
% LICENSE: Copyright 2016, 2017, All Rights Reserved                      %
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

classdef autogen_max11644eua_local_data_t < handle
  properties (Access = public, Constant = true)
    type(1,1) autogen_type_t = autogen_type_t.MAX11644EUA_LOCAL_DATA;
  end
  properties (Access = public)
    topic(1,1) autogen_topic_max11644eua_local_data_t = autogen_topic_max11644eua_local_data_t.UNDEFINED;
    node(1,1) autogen_node_t = autogen_node_t.UNDEFINED;
    ts(1,1) uint64;
    counts_to_volts(1,1) single;
  end
  methods (Static)
    function [n] = size()
      n = 11+4;
  ...
  methods (Sealed = true)
    ...
    function [buffer,i2] = serialize_content(obj,buffer,i1)
      ...
      mustBePositive(i1)
      if (all(isa(buffer,'uint8')))&&(isvector(buffer))
        buffer(i1+(0:(0+3))) = typecast(obj.counts_to_volts,'uint8');
        i2 = i1+4;
      else
        error('Buffer must be a uint8 array.')
      end
    ...
    function [i2] = deserialize_content(obj,buffer,i1)
      ...
      mustBePositive(i1)
      if (all(isa(buffer,'uint8')))&&(isvector(buffer))
        obj.counts_to_volts = typecast(buffer(i1+(0:(0+3))),'single');
        i2 = i1+4;
      else
        error('Buffer must be a uint8 array.')
      end
