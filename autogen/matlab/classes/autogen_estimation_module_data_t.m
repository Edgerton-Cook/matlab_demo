%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% TITLE:   autogen/matlab/classes/autogen_estimation_module_data_t.m      %
% AUTHOR:  Miki Szmuk                                                     %
% LAB:     Autonomous Controls Lab (ACL)                                  %
% LICENSE: Copyright 2016, 2017, All Rights Reserved                      %
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

classdef autogen_estimation_module_data_t < handle
  properties (Access = public, Constant = true)
    type(1,1) autogen_type_t = autogen_type_t.ESTIMATION_MODULE_DATA;
  end
  properties (Access = public)
    topic(1,1) autogen_topic_estimation_module_data_t = autogen_topic_estimation_module_data_t.UNDEFINED;
    node(1,1) autogen_node_t = autogen_node_t.UNDEFINED;
    ts(1,1) uint64;
    state(1,1) autogen_estimation_state_t;
    sig(1,1) autogen_estimation_sig_data_t;
    local(1,1) autogen_estimation_local_data_t;
    config(1,1) autogen_estimation_config_data_t;
  end
  methods (Static)
    function [n] = size()
      n = 11+329;
  ...
  methods (Sealed = true)
    ...
    function [buffer,i2] = serialize_content(obj,buffer,i1)
      ...
      if (all(isa(buffer,'uint8')))&&(isvector(buffer))
        ...
        buffer = obj.sig.serialize_content(buffer,i1+1);
        buffer = obj.local.serialize_content(buffer,i1+89);
        buffer = obj.config.serialize_content(buffer,i1+309);
        i2 = i1+329;
      else
        error('Buffer must be a uint8 array.')
      end
    ...
    function [i2] = deserialize_content(obj,buffer,i1)
      ...
      if (all(isa(buffer,'uint8')))&&(isvector(buffer))
        ...
        obj.sig.deserialize_content(buffer,i1+1);
        obj.local.deserialize_content(buffer,i1+89);
        obj.config.deserialize_content(buffer,i1+309);
        i2 = i1+329;
      else
        error('Buffer must be a uint8 array.')
      end
