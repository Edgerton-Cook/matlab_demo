function [L_imp] = mp_loopshape_s3_ns_pm35m(f_co,varargin)

% Define ideal loop shape.
L_cas = cell(0,1);
L_cas{end+1,1} = zpk([],[0 0 0],1);
L_cas{end+1,1} = mp_ctrl_complex_zero(0.25*f_co,0.75,1);
L_cas{end+1,1} = mp_ctrl_lag(0.25*f_co,1.00,1);
L_cas{end+1,1} = mp_ctrl_lag(0.50*f_co,1.15,1);
L_cas{end+1,1} = mp_ctrl_lag(     f_co,1.37,1);
L_cas{end+1,1} = mp_ctrl_lag(2.00*f_co,1.10,1);
L_cas{end+1,1} = mp_ctrl_complex_zero(2.55*f_co,0.65,1)*...
