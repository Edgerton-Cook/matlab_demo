function [L_imp] = mp_loopshape_s1_as_pm50(f_co,varargin)

% Define ideal loop shape.
L_cas = cell(0,1);
L_cas{end+1,1} = zpk([],0,1);
L_cas{end+1,1} = mp_ctrl_pole(0.50*f_co,1);
L_cas{end+1,1} = mp_ctrl_zero(1.00*f_co,1);
L_cas{end+1,1} = mp_ctrl_pole(10.00*f_co,1);
